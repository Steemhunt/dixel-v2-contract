...
abstract contract SVGGenerator is Constants {
    ...
    // MARK: - Using block-stacking approach with color variables
    // Better speed, Worse svg size (8.71KB)
    ...
    // MARK: - Using paths for each palette color
    // Worse speed, Better svg size (4.58KB)

    string private constant HEADER2 = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 -0.5 24 24" shape-rendering="crispEdges">';
    string private constant FOOTER2 = '</svg>';

    function _generateSVG2(uint24[PALETTE_SIZE] memory palette, uint8[TOTAL_PIXEL_COUNT] memory pixels) internal pure returns (string memory) {
        ...
        for (uint256 x = 0; x < CANVAS_SIZE; x++) {
            for (uint256 y = 0; y < CANVAS_SIZE; y++) {
                uint256 current = pixels[x + y * CANVAS_SIZE];
                width++;

                if (x != 0 || y != 0 || current != prev) {
                    if (bytes(paths[prev]).length != 0 && width > 0) {
                ...
                if (x == CANVAS_SIZE - 1 && y == CANVAS_SIZE - 1) {
        ...
        return string(abi.encodePacked(HEADER2, joined, FOOTER2));
    }
